...
```
    ...
    <logger name="www.qingxiangyx.litemall.core" level="DEBUG" />
    <logger name="www.qingxiangyx.litemall.db" level="DEBUG" />
    <logger name="www.qingxiangyx.litemall.admin" level="DEBUG" />
    <logger name="www.qingxiangyx.litemall.wx" level="DEBUG" />
    <logger name="www.qingxiangyx.litemall" level="DEBUG" />
```

具体如何配置，请自行学习Spring Boot的日志配置和logback日志配置。

`www.qingxiangyx.litemall.core`定义litemall-core模块的日志级别
`www.qingxiangyx.litemall.db`定义litemall-db模块的日志级别
`www.qingxiangyx.litemall.wx`定义litemall-wx-api模块的日志级别
`www.qingxiangyx.litemall.admin`定义litemall-admin-api模块的日志级别
`www.qingxiangyx.litemall`而定义litemall所有后端模块的日志级别
